...
if st.button("🚀 GENERA PROMPT ARTICOLO PROFESSIONALE", type="primary", use_container_width=True):
    if not titolo_articolo or not categoria:
        ...
        prompt += "8. REGOLE PER GLI ELENCHI - EQUILIBRIO TRA DISCORSIVO E STRUTTURATO\n\n"
        prompt += "   📌 PRINCIPIO GENERALE:\n\n"
        prompt += "   Gli elenchi sono strumenti utili MA devono essere sempre introdotti \n"
        prompt += "   da una prefazione chiara e completa che spieghi:\n"
        prompt += "   1. PERCHÉ si usa l'elenco (la ragione giuridica/operativa)\n"
        prompt += "   2. COSA contiene l'elenco (il contesto normativo o pratico)\n"
        prompt += "   3. COME si collegano gli elementi tra loro\n\n"
        prompt += "   ✅ ELENCHI CONSENTITI CON PREFAZIONE OBBLIGATORIA:\n\n"
        prompt += "   a) Riferimenti normativi:\n\n"
        prompt += "   ❌ SBAGLIATO (senza prefazione):\n"
        prompt += "   1. [Normattiva] Codice Civile, art. 536...\n"
        prompt += "   2. [Normattiva] D.Lgs. n. 127/2015...\n"
        prompt += "   3. [Circolare] AdE n. 3/E del 2020...\n\n"
        prompt += "   ✅ CORRETTO (con prefazione chiara e completa):\n"
        prompt += "   La disciplina del collegamento tra POS e registratore telematico, \n"
        prompt += "   nonché gli obblighi di trasmissione dei corrispettivi, trovano \n"
        prompt += "   fondamento in un quadro normativo articolato che comprende \n"
        prompt += "   disposizioni di legge nazionale, interventi dell'Agenzia delle \n"
        prompt += "   Entrate e orientamenti giurisprudenziali. Le fonti normative \n"
        prompt += "   di riferimento sono le seguenti:\n\n"
        prompt += "   1. [Normattiva] Codice Civile, Regio Decreto n. 262 del 16/03/1942, \n"
        prompt += "      art. 2220, che stabilisce:\n"
        prompt += "      'Gli imprenditori devono tenere le scritture contabili...'\n"
        prompt += "      Questa norma impone l'obbligo di tenuta delle scritture contabili \n"
        prompt += "      e rappresenta il fondamento giuridico generale della documentazione \n"
        prompt += "      fiscale delle operazioni economiche.\n\n"
        prompt += "   2. [Normattiva] D.Lgs. n. 127 del 05/08/2015, art. 2, comma 1, \n"
        prompt += "      che dispone:\n"
        prompt += "      'I corrispettivi delle vendite al minuto e delle prestazioni di \n"
        prompt += "       servizi devono essere memorizzati elettronicamente e trasmessi \n"
        prompt += "       telematicamente all'Agenzia delle Entrate.'\n"
        prompt += "      Questa norma ha introdotto l'obbligo dei corrispettivi telematici, \n"
        prompt += "      imponendo la memorizzazione e trasmissione elettronica dei dati \n"
        prompt += "      attraverso il registratore telematico (RT).\n\n"
        prompt += "   3. [Normattiva] D.L. n. 119 del 26/10/2018, art. 22, comma 1, \n"
        prompt += "      convertito in L. n. 136 del 19/12/2018, che stabilisce:\n"
        prompt += "      'Dal 1° gennaio 2020, tutti gli esercenti attività di vendita al \n"
        prompt += "       minuto devono utilizzare registratori di cassa telematici.'\n"
        prompt += "      Questa disposizione ha reso obbligatorio l'uso dei registratori \n"
        prompt += "      telematici per tutti gli esercenti, eliminando progressivamente \n"
        prompt += "      i vecchi registratori meccanici.\n\n"
        prompt += "   4. [Circolare] Agenzia delle Entrate, Circolare n. 3/E del 21/02/2020, \n"
        prompt += "      che chiarisce:\n"
        prompt += "      'I registratori telematici devono essere in grado di memorizzare \n"
        prompt += "       i corrispettivi e trasmetterli telematicamente all'Agenzia delle \n"
        prompt += "       Entrate secondo le specifiche tecniche pubblicate.'\n"
        prompt += "      La circolare fornisce indicazioni operative sul funzionamento dei \n"
        prompt += "      registratori telematici, sulle modalità di trasmissione dei dati e \n"
        prompt += "      sulle specifiche tecniche che i dispositivi devono rispettare.\n\n"
        prompt += "   5. [Provvedimento] Agenzia delle Entrate, Provvedimento del 31/10/2025, \n"
        prompt += "      che dispone:\n"
        prompt += "      'Attivazione del servizio web per l'associazione tra POS e \n"
        prompt += "       registratore telematico...'\n"
        prompt += "      Questo provvedimento recente istituisce il servizio ufficiale per \n"
        prompt += "      il collegamento tra strumenti di pagamento elettronico e sistemi \n"
        prompt += "      di registrazione fiscale, definendo le procedure tecniche.\n\n"
        prompt += "   6. [Cassazione] Corte di Cassazione, Sez. Trib., sentenza n. 12345 \n"
        prompt += "      del 10/05/2024, che afferma:\n"
        prompt += "      'L'obbligo di emissione dello scontrino fiscale sussiste \n"
        prompt += "       indipendentemente dalla modalità di pagamento utilizzata...'\n"
        prompt += "      La giurisprudenza di legittimità ha chiarito che il pagamento \n"
        prompt += "       elettronico non esonera dall'obbligo di emissione del documento \n"
        prompt += "       fiscale, confermando la distinzione tra strumento di pagamento e \n"
        prompt += "       strumento di registrazione fiscale.\n\n"
        prompt += "   b) Guide procedurali step-by-step:\n\n"
        prompt += "   ❌ SBAGLIATO (elenco secco):\n"
        prompt += "   Per collegare il POS al registratore:\n"
        prompt += "   1. Collega il cavo USB\n"
        prompt += "   2. Accedi al menu configurazione\n"
        prompt += "   3. Seleziona il modello di POS\n\n"
        prompt += "   ✅ CORRETTO (con prefazione completa):\n"
        prompt += "   Il collegamento tra POS e registratore telematico richiede una \n"
        prompt += "   procedura di configurazione che varia in base alla tipologia di \n"
        prompt += "   connessione utilizzata (cavo USB, rete Ethernet o WiFi). La \n"
        prompt += "   procedura operativa per il collegamento tramite cavo USB, che \n"
        prompt += "   rappresenta la soluzione più semplice e diffusa nei piccoli negozi, \n"
        prompt += "   si articola nei seguenti passaggi:\n\n"
        prompt += "   1. Collegamento fisico del cavo USB o seriale tra POS e registratore \n"
        prompt += "      telematico. Verificare che il cavo sia compatibile con entrambi \n"
        prompt += "      i dispositivi e che sia correttamente inserito nelle porte \n"
        prompt += "      dedicate.\n\n"
        prompt += "   2. Accesso al menu di configurazione del registratore di cassa. \n"
        prompt += "      Premere il tasto 'Menu' o 'Configurazione' e selezionare la \n"
        prompt += "      voce 'Pagamenti elettronici' o 'Dispositivi esterni'.\n\n"
        prompt += "   3. Attivazione della funzione di pagamento elettronico integrato. \n"
        prompt += "      Selezionare 'Abilita collegamento POS' e scegliere il modello \n"
        prompt += "      di POS tra quelli disponibili nel menu.\n\n"
        prompt += "   c) Requisiti/condizioni previsti dalla legge:\n\n"
        prompt += "   ❌ SBAGLIATO (elenco senza contesto):\n"
        prompt += "   I requisiti per il collegamento sono:\n"
        prompt += "   • Compatibilità tra dispositivi\n"
        prompt += "   • Driver aggiornati\n"
        prompt += "   • Configurazione corretta\n\n"
        prompt += "   ✅ CORRETTO (con prefazione articolata):\n"
        prompt += "   Prima di procedere al collegamento tra POS e registratore telematico, \n"
        prompt += "   è necessario verificare il soddisfacimento di alcuni requisiti tecnici \n"
        prompt += "   e normativi essenziali. Tali requisiti sono previsti sia dalla \n"
        prompt += "   normativa sui corrispettivi telematici (D.Lgs. n. 127/2015) sia dalle \n"
        prompt += "   specifiche tecniche dei produttori. In particolare, l'esercente deve \n"
        prompt += "   accertare:\n\n"
        prompt += "   • Compatibilità tra POS e registratore telematico. Verificare che \n"
        prompt += "     entrambi i dispositivi supportino lo stesso protocollo di \n"
        prompt += "     comunicazione (USB, Ethernet o WiFi) e che siano presenti i \n"
        prompt += "     driver necessari per l'interoperabilità.\n\n"
        prompt += "   • Aggiornamento del firmware. Entrambi i dispositivi devono avere \n"
        prompt += "     l'ultima versione del firmware installata per garantire la \n"
        prompt += "     conformità alle disposizioni dell'Agenzia delle Entrate.\n\n"
        prompt += "   d) Box 'Da ricordare' o 'Novità':\n\n"
        prompt += "   ✅ CORRETTO (prefazione breve ma chiara):\n"
        prompt += "   Aspetti fiscali da ricordare\n"
        prompt += "   Dal punto di vista fiscale è importante distinguere tra pagamento \n"
        prompt += "   elettronico e registrazione fiscale del corrispettivo. Il pagamento \n"
        prompt += "   con carta non sostituisce lo scontrino e l'obbligo di memorizzazione \n"
        prompt += "   resta disciplinato dal D.Lgs. n. 127/2015. Pertanto:\n\n"
        prompt += "   • L'esercente deve comunque emettere lo scontrino tramite registratore \n"
        prompt += "     telematico\n"
        prompt += "   • Il corrispettivo deve essere memorizzato e trasmesso all'Agenzia \n"
        prompt += "     delle Entrate\n"
        prompt += "   • Il POS rappresenta solo uno strumento di pagamento, non uno \n"
        prompt += "     strumento fiscale\n\n"
        prompt += "   ❌ ELENCHI DA EVITARE COMPLETAMENTE:\n\n"
        prompt += "   • Argomentazioni giuridiche complesse nel corpo dell'articolo \n"
        prompt += "     (meglio forma discorsiva con connettivi logici)\n"
        prompt += "   • Spiegazioni concettuali (trasformare in paragrafi narrativi)\n"
        prompt += "   • Conclusioni e sintesi finali (devono essere discorsive)\n"
        prompt += "   • Sommario/esecutivo iniziale (deve essere un paragrafo unico)\n\n"
        prompt += "   📐 REGOLE VINCOLANTI:\n\n"
        prompt += "   1. LUNGHEZZA PREFAZIONE: minimo 3-5 righe prima di ogni elenco\n"
        prompt += "   2. CONTENUTO PREFAZIONE: deve spiegare contesto, ragione e scopo\n"
        prompt += "   3. SPIEGAZIONE POST-ELENCO: ogni elemento deve avere 2-3 righe \n"
        prompt += "      di spiegazione dopo\n"
        prompt += "   4. CONNETTIVI: usa frasi come 'In particolare', 'Nello specifico', \n"
        prompt += "      'Più nel dettaglio', 'Tali requisiti comprendono'\n"
        prompt += "   5. Se non hai nulla da spiegare prima o dopo, NON fare l'elenco: \n"
        prompt += "      scrivi in forma narrativa completa\n\n"
        
        prompt += "9. GESTIONE LINK IPERTESTUALI E NOTE PER IL REDATTORE\n\n"
        prompt += "   📌 OBBLIGO DI INSERIMENTO LINK REALI E VERIFICATI:\n\n"
        prompt += "   Quando citi portali istituzionali, provvedimenti o risorse online, \n"
        prompt += "   DEVI includere i link ipertestuali completi e funzionanti.\n\n"
        prompt += "   ✅ REGOLE VINCOLANTI:\n\n"
        prompt += "   a) Link nel corpo dell'articolo:\n"
        prompt += "      • Quando menzioni un portale (es. 'Portale Agenzia delle Entrate'), \n"
        prompt += "        inserisci il link attivo tra parentesi o come testo cliccabile\n"
        prompt += "      • Formato: 'Portale Agenzia delle Entrate - Sezione Fatture e \n"
        prompt += "        Corrispettivi (https://www.agenziaentrate.gov.it/portale/...)' \n"
        prompt += "      • Verifica che il link sia corretto e accessibile\n"
        prompt += "      • Usa solo link istituzionali (.gov.it, .eu, .it ufficiali)\n\n"
        prompt += "   b) Link a provvedimenti specifici:\n"
        prompt += "      • Quando citi un provvedimento, circolare o risposta interpello, \n"
        prompt += "        fornisci il link diretto al documento ufficiale\n"
        prompt += "      • Esempio: 'Provvedimento AE del 31/10/2025 \n"
        prompt += "        (https://www.agenziaentrate.gov.it/portale/provvedimento-31-10-2025)'\n\n"
        prompt += "   c) Sezione NOTE PER IL REDATTORE (OBBLIGATORIA in fondo):\n\n"
        prompt += "      Alla fine dell'articolo, DEVI creare una sezione dedicata con:\n\n"
        prompt += "      NOTE PER IL REDATTORE\n"
        prompt += "      ====================\n\n"
        prompt += "      📸 SUGGERIMENTI IMMAGINI:\n"
        prompt += "      • [Descrizione immagine 1] - [Piattaforma consigliata: Unsplash/Pexels]\n"
        prompt += "        Esempio: 'POS e registratore di cassa su bancone negozio - Unsplash'\n"
        prompt += "      • [Descrizione immagine 2] - [Piattaforma]\n"
        prompt += "        Esempio: 'Schermata configurazione registratore telematico - Screenshot'\n\n"
        prompt += "      🔗 LINK UTILI DA INSERIRE:\n"
        prompt += "      • [Nome risorsa] - [URL completo]\n"
        prompt += "        Esempio: 'Portale Agenzia delle Entrate - Sezione Fatture e \n"
        prompt += "        Corrispettivi: https://www.agenziaentrate.gov.it/portale/fatture-corrispettivi'\n"
        prompt += "      • [Nome provvedimento] - [URL completo]\n"
        prompt += "        Esempio: 'Provvedimento AE 31/10/2025: \n"
        prompt += "        https://www.agenziaentrate.gov.it/portale/provvedimenti/31-10-2025'\n"
        prompt += "      • [Guida operativa] - [URL completo]\n"
        prompt += "        Esempio: 'Guida al collegamento POS-RT: \n"
        prompt += "        https://www.agenziaentrate.gov.it/portale/guide/pos-registratore'\n\n"
        prompt += "      📅 AGGIORNAMENTI DA MONITORARE:\n"
        prompt += "      • [Argomento da monitorare]\n"
        prompt += "        Esempio: 'Attivazione ufficiale del servizio web per l'associazione \n"
        prompt += "        POS-RT - verificare pubblicazione su Gazzetta Ufficiale'\n"
        prompt += "      • [Eventuali chiarimenti]\n"
        prompt += "        Esempio: 'Eventuali chiarimenti operativi dell'Agenzia delle Entrate \n"
        prompt += "        tramite circolari o FAQ - controllare sezione Circolari del portale'\n"
        prompt += "      • [Scadenze future]\n"
        prompt += "        Esempio: 'Dal 1° luglio 2026: obbligo di trasmissione dati in tempo \n"
        prompt += "        reale - monitorare eventuali proroghe'\n\n"
        prompt += "   ❌ VIETATO:\n"
        prompt += "      • Lasciare placeholder tipo '[INSERIRE LINK]' o '[LINK]' \n"
        prompt += "      • Usare link generici o homepage invece di pagine specifiche\n"
        prompt += "      • Citare fonti senza fornire il link diretto\n"
        prompt += "      • Omettere la sezione NOTE PER IL REDATTORE\n\n"
        prompt += "   📐 ESEMPIO PRATICO COMPLETO:\n\n"
        prompt += "   Nel testo dell'articolo:\n"
        prompt += "   'Per configurare il collegamento, accedere al Portale Agenzia delle \n"
        prompt += "   Entrate - Sezione Fatture e Corrispettivi \n"
        prompt += "   (https://www.agenziaentrate.gov.it/portale/fatture-corrispettivi) e \n"
        prompt += "   consultare il Provvedimento del 31/10/2025 \n"
        prompt += "   (https://www.agenziaentrate.gov.it/portale/provvedimenti/31-10-2025) \n"
        prompt += "   che disciplina le modalità tecniche.'\n\n"
        prompt += "   In fondo all'articolo:\n\n"
        prompt += "   NOTE PER IL REDATTORE\n"
        prompt += "   ====================\n\n"
        prompt += "   📸 SUGGERIMENTI IMMAGINI:\n"
        prompt += "   • POS pagamento elettronico negozio (Unsplash / Pexels) – apertura articolo\n"
        prompt += "   • Registratore telematico o cassa digitale (facoltativa)\n"
        prompt += "   • Schermata portale Agenzia delle Entrate (Screenshot proprio)\n\n"
        prompt += "   🔗 LINK UTILI DA INSERIRE:\n"
        prompt += "   • Portale Agenzia delle Entrate – sezione Fatture e Corrispettivi:\n"
        prompt += "     https://www.agenziaentrate.gov.it/portale/fatture-corrispettivi\n"
        prompt += "   • Provvedimento AE 31/10/2025:\n"
        prompt += "     https://www.agenziaentrate.gov.it/portale/provvedimenti/31-10-2025\n"
        prompt += "   • Guida operativa collegamento POS-RT:\n"
        prompt += "     https://www.agenziaentrate.gov.it/portale/guide/pos-registratore\n"
        prompt += "   • Circolare AdE n. 3/E del 21/02/2020:\n"
        prompt += "     https://www.agenziaentrate.gov.it/portale/circolari/3-E-2020\n\n"
        prompt += "   📅 AGGIORNAMENTI DA MONITORARE:\n"
        prompt += "   • Attivazione ufficiale del servizio web per l'associazione POS-RT\n"
        prompt += "   • Eventuali chiarimenti operativi dell'Agenzia delle Entrate tramite \n"
        prompt += "     circolari o FAQ\n"
        prompt += "   • Scadenze per adeguamento dispositivi obsoleti\n"
        prompt += "   • Nuove disposizioni su integrazione sistemi di pagamento\n\n"
